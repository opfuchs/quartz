...
This is where *detections* or *detection logic* come(s) in. These attempt to correlate - remember, before we discussed how this is inherently uncertain - particular pieces of telemetry with more general behaviors. For example, on Windows, this might be something like:
...
To detect an attempt to hide code injection in `conhost.exe` (the Console Window Host).[^3][^4]
